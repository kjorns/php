...
if (!isset($_GET['article_id'])) {
  header('Location: /php/final-project/admin/index.php');
  exit;
}
...
if (isset($_GET['article_id']) && !$_POST) {
  ...
  $sql = 'SELECT article_id, image_id, title, article
          FROM php_fp_blog WHERE article_id = ?';
  if ($stmt->prepare($sql)) {
...
if ( (isset($_POST['insert'])) && ($error === '') ) {
  ...
  if (!isset($imageError)) {
    ...
    if (isset($image_id)) {
      $sql = 'UPDATE php_fp_blog SET image_id = ?, title = ?, article = ?
               WHERE article_id = ?';
      $stmt->prepare($sql);
      $stmt->bind_param('issi', $image_id, $_POST['title'], $_POST['article'], $_POST['article_id'] );
    } else {
      $sql = 'UPDATE php_fp_blog SET image_id = NULL, title = ?, article = ?
              WHERE article_id = ?';
      $stmt->prepare($sql);
...
<main>
        ...
        <h2>
            ...
            <p><a href="index.php">List All Entries </a></p>
            <?php 
            if (isset($error)) {
