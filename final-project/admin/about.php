...
if ( (isset($_POST['insert'])) && ($error === '') ) {
  ...
  if(isset($_POST['upload_new']) && $_FILES['image']['error'] == 0) {
    ...
    try {
        $loader = new ThumbnailUpload($_SERVER['DOCUMENT_ROOT'] . '/php/final-project/images/');
        $loader->setThumbDestination($_SERVER['DOCUMENT_ROOT'] . '/php/final-project/images/thumbs/');
    ...
    if ($names) {
      $sql = 'INSERT INTO php_fp_images (filename, caption)
              VALUES (?, ?)';
      $stmt->prepare($sql);
  ...
  if (!isset($imageError)) {
    ...
    if (isset($image_id)) {
      $sql = 'INSERT INTO php_fp_about_us (image_id, title, article, created)
              VALUES(?, ?, ?, NOW())';
      $stmt->prepare($sql);
      $stmt->bind_param('iss', $image_id, $_POST['title'], $_POST['article']);
    } else {
      // create SQL
      $sql = 'INSERT INTO php_fp_about_us (title, article, created)
              VALUES(?, ?, NOW())';
      $stmt->prepare($sql);
  ...
  if ($OK && !isset($imageError)) {
    header('Location: /php/final-project/about-us/index.php');
    exit;
  } else {
    $error = $stmt->error;
    if (isset($imageError)) {
    ...
    <ol>
        ...
        <li>
            ...
            $getImages = 'SELECT image_id, filename
                         FROM php_fp_images ORDER BY filename';
...
<?php include('../includes/logout.php'); ?>
    ...
    <aside>
        ...
        <nav>
            <ul>
                <li><a href="/php/final-project/admin/index.php">Home</a></li>
                <li><a class="current" href="/php/final-project/admin/about.php">About</a></li>
